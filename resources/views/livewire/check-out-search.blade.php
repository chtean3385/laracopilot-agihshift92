<div class="space-y-5">
    ...
    <div class="rounded-2xl p-6 text-white" style="background: linear-gradient(135deg, #1a2332, #2a3545);">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p style="color: rgba(201,169,110,.6);">{{ $pendingCheckouts->total() }} guest(s) awaiting check-out</p>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 card-hover">
            ...
            <div class="space-y-2 mb-4">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    @endphp
                    @if($coRoomPT === 'per_hour')
                    <span class="font-bold" style="color: #c9a96e;"><i class="fas fa-clock mr-1 text-xs"></i>Billed at checkout</span>
                    @else
                    <span class="font-bold {{ $coBalance > 0 ? 'text-red-500' : 'text-green-600' }}">₹{{ number_format($coBalance) }}</span>
                    @endif
                </div>
                @if($coRoomPT === 'per_hour' && $booking->actual_checkin_at)
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500">Checked In At</span>
                    <span class="font-semibold" style="color: #c9a96e;">{{ $booking->actual_checkin_at->format('h:i A') }}</span>
                </div>
                @endif
            </div>
            <a href="{{ route('checkout.show', $booking->id) }}" class="w-full text-center block text-white py-2.5 rounded-xl font-medium text-sm transition-all" style="background: linear-gradient(135deg, #1a2332, #2a3545);" onmouseover="this.style.background='linear-gradient(135deg, #2a3545, #3a4555)'" onmouseout="this.style.background='linear-gradient(135deg, #1a2332, #2a3545)'">
    ...
    <div class="bg-white rounded-2xl p-16 text-center shadow-sm border border-gray-100">
        <div class="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4" style="background: rgba(201,169,110,.1);">
            <i class="fas fa-check-double text-3xl" style="color: #c9a96e;"></i>
        </div>
