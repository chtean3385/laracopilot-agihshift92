    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2 flex-wrap">
            ...
            @if($booking->status == 'checked_in')
                <a href="{{ route('checkout.show', $booking->id) }}" class="text-white px-5 py-2.5 rounded-xl font-medium text-sm" style="background: linear-gradient(135deg, #c9a96e, #b08d56);"><i class="fas fa-sign-out-alt mr-2"></i>Process Check-Out</a>
            @endif
            @if($booking->status === 'checked_in' && $booking->checkout_token)
                <button onclick="document.getElementById('bkGuestQrModal').style.display='flex'"
                    style="display:inline-flex;align-items:center;gap:6px;padding:9px 16px;background:linear-gradient(135deg,#c9a96e,#b08d56);color:#fff;border:none;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;">
            ...
            <button onclick="fillPathik()" style="display:inline-flex;align-items:center;gap:6px;padding:9px 16px;background:linear-gradient(135deg,#1a2332,#2a3545);color:#fff;border:none;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-5">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between"><span class="text-sm text-gray-500">Booking #</span><span class="text-sm font-mono font-bold" style="color: #c9a96e;">{{ $booking->booking_number }}</span></div>
                    ...
                    @if($pType === 'per_slot' && $booking->booking_date)
                        ...
                        <div class="flex justify-between"><span class="text-sm text-gray-500">Time Slot</span>
                            <span class="text-sm font-semibold" style="color: #c9a96e;">
                                @if($booking->timeSlot)
                    ...
                    @elseif($pType === 'per_hour' && $booking->booking_date)
                        ...
                        <div class="flex justify-between"><span class="text-sm text-gray-500">Start Time</span><span class="text-sm font-semibold" style="color: #c9a96e;">{{ $booking->slot_start_time ?? '—' }}</span></div>
                        <div class="flex justify-between"><span class="text-sm text-gray-500">Duration</span><span class="text-sm font-semibold" style="color: #c9a96e;">{{ $booking->hours_booked ?? '—' }} hrs</span></div>
                    @else
                ...
                <div class="mt-4 p-3 rounded-xl" style="background: rgba(201,169,110,.08); border: 1px solid rgba(201,169,110,.15);">
                    <p class="text-xs font-semibold" style="color: #b08d56;"><i class="fas fa-star mr-1"></i>Special Requests</p>
                    <p class="text-sm mt-1" style="color: #c9a96e;">{{ $booking->special_requests }}</p>
                </div>
                @endif
                @if($booking->checkin_notes)
                <div class="mt-3 p-3 rounded-xl" style="background: rgba(201,169,110,.08); border: 1px solid rgba(201,169,110,.15);">
                    <p class="text-xs font-semibold" style="color: #b08d56;"><i class="fas fa-sign-in-alt mr-1"></i>Check-In Notes</p>
                    <p class="text-sm mt-1" style="color: #c9a96e;">{{ $booking->checkin_notes }}</p>
                </div>
                @endif
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold" style="background: linear-gradient(135deg, #c9a96e, #b08d56);">{{ substr($booking->customer?->name ?? '(Deleted Guest)', 0, 1) }}</div>
                    <div>
                ...
                <a href="{{ route('customers.show', $booking->customer_id) }}" class="text-sm hover:underline" style="color: #c9a96e;"><i class="fas fa-external-link-alt mr-1"></i>View Guest Profile</a>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <h3 class="font-bold text-gray-800 mb-4">
                        ...
                        <span class="ml-2 inline-flex items-center gap-1 px-2 py-0.5 text-xs font-bold rounded-full" style="color: #1a2332; background: rgba(201,169,110,.15); border: 1px solid rgba(201,169,110,.2);"><i class="fas fa-hotel" style="color: #c9a96e;"></i> Whole Hotel</span>
                        @endif
                    </h3>
                    @if($booking->is_whole_hotel)
                    <div class="flex flex-col items-center justify-center py-4 text-center">
                        <div class="w-14 h-14 rounded-full flex items-center justify-center mb-3" style="background: rgba(201,169,110,.1);">
                            <i class="fas fa-hotel text-2xl" style="color: #c9a96e;"></i>
                        </div>
                        <p class="text-lg font-black" style="color: #c9a96e;">Whole Hotel / Villa</p>
                        ...
                        <div class="mt-3 space-y-1 w-full text-sm">
                            ...
                            <div class="flex justify-between"><span class="text-gray-500">Custom Total</span><span class="font-bold" style="color: #c9a96e;">₹{{ number_format($booking->total_amount) }}</span></div>
                            @else
                            <div class="flex justify-between"><span class="text-gray-500">Total</span><span class="font-bold" style="color: #c9a96e;">₹{{ number_format($booking->total_amount) }}</span></div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
                        <span style="font-size:13px;font-weight:700;color:#1a2332;background:rgba(201,169,110,.15);padding:3px 10px;border-radius:20px;">
                            <i class="fas fa-layer-group" style="font-size:11px;margin-right:4px;color:#c9a96e;"></i>
                    ...
                    <div class="space-y-3">
                        ...
                        <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:12px;">
                            <div style="display:flex;align-items:center;gap:10px;">
                                <div style="width:36px;height:36px;background:linear-gradient(135deg,#c9a96e,#b08d56);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                            ...
                            <div style="text-align:right;">
                                <div style="font-weight:700;font-size:14px;color:#c9a96e;">₹{{ number_format($gbAmount) }}</div>
                                <div style="font-size:11px;color:#94a3b8;">{{ $booking->nights }} night{{ $booking->nights != 1 ? 's' : '' }}</div>
                            </div>
                        </div>
                        @endforeach
                    </div>
                    <div style="margin-top:12px;padding:10px 12px;background:rgba(201,169,110,.08);border:1.5px solid rgba(201,169,110,.2);border-radius:12px;display:flex;justify-content:space-between;align-items:center;">
                        <span style="font-size:13px;font-weight:700;color:#b08d56;">Combined Total</span>
                        <span style="font-size:16px;font-weight:900;color:#c9a96e;">₹{{ number_format($booking->total_amount) }}</span>
                    </div>
                    @elseif($booking->room)
                    ...
                    <div class="mt-4 space-y-2">
                        ...
                        <div class="flex justify-between text-sm"><span class="text-gray-500">Slot Price</span><span class="font-bold" style="color: #c9a96e;">₹{{ number_format($booking->timeSlot->base_price) }}</span></div>
                        @elseif($pType === 'per_hour')
                        <div class="flex justify-between text-sm"><span class="text-gray-500">Rate/Hour</span><span class="font-bold" style="color: #c9a96e;">₹{{ number_format($booking->room?->hourly_rate ?? 0) }}</span></div>
                        @else
                        @if($booking->price_overridden)
                        <div class="flex justify-between text-sm"><span class="text-gray-500">Custom Total</span><span class="font-bold" style="color: #c9a96e;">₹{{ number_format($booking->total_amount) }} <span class="text-xs font-normal" style="color: #b08d56;">(custom)</span></span></div>
                        @else
                        <div class="flex justify-between text-sm"><span class="text-gray-500">Rate/Night</span><span class="font-bold" style="color: #c9a96e;">₹{{ number_format($booking->room?->price_per_night ?? 0) }}@if($_taxRate > 0)<span class="text-xs font-normal text-gray-400 ml-1">+ {{ $_taxRate }}% GST</span>@endif</span></div>
                        @endif
                        @endif
                    </div>
                    @else
                    <div class="flex flex-col items-center justify-center py-6 text-center">
                        <div class="w-12 h-12 rounded-full flex items-center justify-center mb-3" style="background: rgba(201,169,110,.1);">
                            <i class="fas fa-clock" style="color: #c9a96e;"></i>
                        </div>
                        <p class="text-sm font-semibold" style="color: #c9a96e;">Room Pending Assignment</p>
                        ...
                        <button onclick="document.getElementById('assignRoomModal').style.display='flex'"
                                ...
                                style="background:linear-gradient(135deg,#1a2332,#2a3545);">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500"><i class="fas fa-hotel mr-1" style="color: #c9a96e;"></i>Whole Hotel / Villa</span>
                            <span class="font-medium">₹{{ number_format($booking->total_amount) }}</span>
                        </div>
                        @elseif($pType === 'per_slot' && $booking->timeSlot)
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">
                                <i class="fas fa-clock mr-1" style="color: #c9a96e;"></i>{{ $booking->timeSlot->name }}
                        ...
                        <div class="flex justify-between text-sm items-center">
                            <span class="text-gray-500">
                                <i class="fas fa-hourglass-half mr-1" style="color: #c9a96e;"></i>Hourly booking
                                @if($booking->price_overridden)
                                <span class="inline-flex items-center gap-1 ml-1.5 px-1.5 py-0.5 rounded-md text-xs font-semibold" style="background: rgba(201,169,110,.1); color: #b08d56; border: 1px solid rgba(201,169,110,.15);">
                        ...
                        @elseif($booking->price_overridden)
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500"><i class="fas fa-pen mr-1" style="color: #c9a96e;"></i>Room charge <span class="text-xs" style="color: #c9a96e;">(custom price)</span></span>
                        ...
                        @endphp
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500"><i class="fas fa-layer-group mr-1" style="color: #c9a96e;"></i>{{ $booking->groupedBookings->count() + 1 }} rooms · {{ $booking->nights }} night{{ $booking->nights != 1 ? 's' : '' }}</span>
                        ...
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500"><i class="fas fa-utensils mr-1" style="color: #c9a96e;"></i>Meal Plan
                                @if($booking->meal_breakfast)<span class="ml-1 text-xs rounded px-1" style="background: rgba(201,169,110,.1); color: #b08d56;">B</span>@endif
                                @if($booking->meal_lunch)<span class="ml-1 text-xs rounded px-1" style="background: rgba(201,169,110,.1); color: #b08d56;">L</span>@endif
                                @if($booking->meal_dinner)<span class="ml-1 text-xs rounded px-1" style="background: rgba(201,169,110,.1); color: #b08d56;">D</span>@endif
                            </span>
                            <span class="font-medium" style="color: #c9a96e;">₹{{ number_format($booking->meal_cost) }}</span>
                        </div>
                        @endif
                        @if(!$booking->groupedBookings->isNotEmpty() && $booking->extra_beds > 0)
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500"><i class="fas fa-bed mr-1" style="color: #7a8a9a;"></i>Extra Beds ({{ $booking->extra_beds }})</span>
                            <span class="font-medium" style="color: #7a8a9a;">₹{{ number_format($booking->extra_bed_cost) }}</span>
                        </div>
                        @endif
                        @if($bExtraChargesTotal > 0)
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500"><i class="fas fa-utensils mr-1" style="color: #c9a96e;"></i>Food &amp; Extra</span>
                            <span class="font-medium" style="color: #c9a96e;">₹{{ number_format($bExtraChargesTotal) }}</span>
                        </div>
                        @endif
                        ...
                        <div class="flex justify-between text-sm"><span class="text-gray-500">Total Paid</span><span class="font-semibold" style="color: #c9a96e;">₹{{ number_format($bTotalPaid) }}</span></div>
                        @if($bOverpaid > 0)
                        <div class="flex justify-between text-sm"><span class="text-gray-500" style="color: #b08d56;">Overpayment/Credit</span><span class="font-semibold" style="color: #b08d56;">₹{{ number_format($bOverpaid) }}</span></div>
                        @endif
                        <div class="flex justify-between text-sm border-t pt-2"><span class="font-semibold">Balance Due</span><span class="font-bold {{ $bBalance > 0 ? 'text-red-500' : '' }}" style="{{ $bBalance <= 0 ? 'color: #c9a96e;' : '' }}">₹{{ number_format($bBalance) }}</span></div>
                    </div>
                    <div class="mt-3 flex items-center gap-2 flex-wrap">
                        <span class="badge-{{ $booking->payment_status_color }}">{{ ucfirst($booking->payment_status) }}</span>
                        @if($booking->price_overridden)
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold" style="background: rgba(201,169,110,.1); color: #b08d56; border: 1px solid rgba(201,169,110,.15);">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <a href="{{ route('payments.create') }}" class="hover:underline text-sm" style="color: #c9a96e;">+ Add Payment</a>
                </div>
                ...
                <div class="divide-y divide-gray-50">
                    ...
                    <div class="px-6 py-4 flex items-center justify-between">
                        ...
                        <div class="text-right">
                            <div class="font-bold" style="color: #c9a96e;">₹{{ number_format($payment->amount) }}</div>
    ...
    <div class="divide-y divide-gray-50">
        ...
        <div class="px-6 py-3 flex items-center justify-between">
            ...
            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold
                " style="{{ $ref->verified ? 'background: rgba(201,169,110,.1); color: #b08d56;' : 'background: rgba(201,169,110,.1); color: #b08d56;' }}">
    ...
    <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
        ...
        <form method="POST" action="{{ route('admin.booking-widget.confirm', $booking->id) }}">
            ...
            <div class="mb-4">
                ...
                <p class="text-xs mt-1" style="color: #c9a96e;"><i class="fas fa-exclamation-triangle mr-1"></i>No available rooms right now. Update room status first.</p>
                @endif
            </div>
            <div class="flex gap-3">
                <button type="submit" class="flex-1 py-2 text-sm font-semibold text-white rounded-lg" style="background:linear-gradient(135deg,#1a2332,#2a3545);">
