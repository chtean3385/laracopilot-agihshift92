...
{{-- ── Owner Booking Alerts — shown for ALL module-enabled hotels ─────────── --}}
@if($moduleActive)
<div style="background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:24px;margin-top:24px;max-width:700px;margin-left:auto;margin-right:auto;">
    ...
    <form method="POST" action="{{ route('whatsapp.setup.save-notify-phones') }}" id="ownerAlertForm">
        ...
        {{-- Toggle — uses <div> not <label> to prevent browser double-toggle --}}
        <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;background:#f9fafb;border:1px solid #e5e7eb;border-radius:10px;margin-bottom:18px;">
            <div>
                ...
                <p style="font-size:12px;color:#6b7280;margin:0;">Uses the <code style="font-size:11px;background:#f3f4f6;padding:2px 5px;border-radius:4px;">booking_alert_owner</code> Meta template. Works with shared or own number.</p>
            </div>
            <div style="position:relative;display:inline-block;width:46px;height:26px;flex-shrink:0;cursor:pointer;" onclick="ownerToggle()">
                <input type="checkbox" name="notify_on_booking" value="1" id="notifyToggle"
                    {{ $config->notify_on_booking ? 'checked' : '' }}
                    style="opacity:0;position:absolute;width:0;height:0;">
                <div id="notifyTrack" style="position:absolute;top:0;left:0;right:0;bottom:0;background:{{ $config->notify_on_booking ? '#7c3aed' : '#d1d5db' }};border-radius:26px;transition:.25s;"></div>
                <div id="notifyThumb" style="position:absolute;height:20px;width:20px;left:{{ $config->notify_on_booking ? '23px' : '3px' }};bottom:3px;background:#fff;border-radius:50%;transition:.25s;"></div>
            </div>
        </div>
        ...
        <div style="margin-top:18px;display:flex;gap:10px;align-items:center;">
            ...
            <span style="font-size:12px;color:#9ca3af;">Sent via shared or own WhatsApp number — template must be approved in Meta.</span>
        </div>
    </form>
</div>
...
function ownerToggle() {
    ...
    const track = document.getElementById('notifyTrack');
    const thumb = document.getElementById('notifyThumb');
    cb.checked  = !cb.checked;
    if (cb.checked) {
        track.style.background = '#7c3aed';
        thumb.style.left       = '23px';
        document.getElementById('phonesSection').style.opacity      = '1';
        document.getElementById('phonesSection').style.pointerEvents = '';
    } else {
        track.style.background = '#d1d5db';
        thumb.style.left       = '3px';
        document.getElementById('phonesSection').style.opacity      = '0.5';
        document.getElementById('phonesSection').style.pointerEvents = 'none';
    }
    document.getElementById('ownerAlertForm').submit();
}
...
function removePhone(btn) {
    ...
    if (rows.length === 1) { btn.closest('.phone-row').querySelector('input').value = ''; return; }
