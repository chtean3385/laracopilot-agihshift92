...
<style>
    ...
    .form-group input,.form-group select{width:100%;padding:9px 12px;border:1px solid #e2e8f0;border-radius:9px;font-size:.88rem;color:#374151;box-sizing:border-box;}
    ...
    .stock-pill{display:inline-flex;align-items:center;gap:6px;background:#f0fdf4;border:1px solid #bbf7d0;color:#16a34a;padding:6px 14px;border-radius:20px;font-size:.85rem;font-weight:700;margin-bottom:20px;}
</style>
...
<div class="form-card">
    <div class="form-title"><i class="fas fa-edit" style="color:#0369a1;margin-right:8px;"></i>Edit: {{ $item->name }}</div>

    <div class="stock-pill">
        <i class="fas fa-cubes"></i>
        Current Stock: {{ number_format($item->current_stock, 2) }} {{ $item->unit }}
        &nbsp;·&nbsp;
        <a href="{{ route('inventory.movements', $item->id) }}" style="color:#0369a1;font-size:.8rem;">View History</a>
    </div>
    ...
    <form method="POST" action="{{ route('inventory.update', $item->id) }}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" value="{{ old('name', $item->name) }}" required maxlength="150">
        </div>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                <label>Unit *</label>
                <input type="text" name="unit" value="{{ old('unit', $item->unit) }}" required maxlength="30">
                <div class="form-hint">Changing the unit doesn't convert existing stock</div>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" name="reorder_level" value="{{ old('reorder_level', $item->reorder_level) }}" min="0" step="0.01">
                <div class="form-hint">Low stock alert threshold</div>
            </div>
            <div class="form-group">
        ...
        <div class="form-footer">
            <button type="submit" class="btn-primary"><i class="fas fa-save" style="margin-right:6px;"></i>Save Changes</button>
