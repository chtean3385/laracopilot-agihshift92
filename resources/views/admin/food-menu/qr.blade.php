...
<div style="padding:24px;">
    ...
    <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:12px;padding:14px 18px;margin-bottom:20px;color:#1e40af;font-size:13px;" class="no-print">
        <i class="fas fa-info-circle"></i> <strong>How it works:</strong> Each QR opens the menu with the room number pre-filled. The general hotel QR (top) lets guests pick their own room — useful for the lobby or restaurant area.
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:16px;" class="qr-grid">
        <div style="background:#fff;border-radius:14px;padding:18px;text-align:center;box-shadow:0 2px 12px rgba(0,0,0,.05);page-break-inside:avoid;border:2px dashed #f97316;">
            ...
            <div class="qr-target" data-url="{{ $baseUrl }}" style="display:flex;justify-content:center;margin-bottom:10px;"></div>
            <div style="font-size:11px;color:#64748b;">Scan to open menu — choose your room</div>
        </div>
        ...
        <div style="background:#fff;border-radius:14px;padding:18px;text-align:center;box-shadow:0 2px 12px rgba(0,0,0,.05);page-break-inside:avoid;">
            ...
            <div class="qr-target" data-url="{{ $baseUrl }}/{{ urlencode($room->room_number) }}" style="display:flex;justify-content:center;margin-bottom:10px;"></div>
            <div style="font-size:11px;color:#64748b;">Scan to view menu &amp; order</div>
        </div>
        @endforeach
    </div>
    @endif
