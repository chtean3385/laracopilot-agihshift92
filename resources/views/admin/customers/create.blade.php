                ...
                <div class="md:col-span-2 border-t border-gray-100 pt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="id_type" id="createIdType" onchange="updateIdPlaceholder('createIdNumber', this.value)" class="form-input @error('id_type') border-red-400 @enderror" required>
                ...
                <div class="md:col-span-2 border-t border-gray-100 pt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" name="arrival_city" value="{{ old('arrival_city') }}" class="form-input" placeholder="e.g. Mumbai">
                        </div>
                        <div>
                            <label class="form-label">Purpose of Travel</label>
                            <select name="travel_reason" class="form-input">
                                <option value="">Select reason</option>
                                <option value="Leisure" {{ old('travel_reason') == 'Leisure' ? 'selected' : '' }}>Leisure / Vacation</option>
                                <option value="Business" {{ old('travel_reason') == 'Business' ? 'selected' : '' }}>Business</option>
                                <option value="Wedding" {{ old('travel_reason') == 'Wedding' ? 'selected' : '' }}>Wedding / Event</option>
                                <option value="Medical" {{ old('travel_reason') == 'Medical' ? 'selected' : '' }}>Medical</option>
                                <option value="Transit" {{ old('travel_reason') == 'Transit' ? 'selected' : '' }}>Transit</option>
                                <option value="Other" {{ old('travel_reason') == 'Other' ? 'selected' : '' }}>Other</option>
                            </select>
                        </div>
                        <div>
                            <label class="form-label">Departing To (City)</label>
                            <input type="text" name="dispatch_city" value="{{ old('dispatch_city') }}" class="form-input" placeholder="e.g. Goa">
                        </div>
                    </div>
                </div>
...
// Set placeholder on page load if type is pre-selected (old() value)
document.addEventListener('DOMContentLoaded', function() {
    const sel = document.getElementById('createIdType');
    if (sel && sel.value) updateIdPlaceholder('createIdNumber', sel.value);
});
</script>
@endsection
