...
| 2026-06-04 | v1.0.31 | `pending` | WA chatbot: intercept "Book Demo"/"Call Me Back" button clicks before state machine (prevents corrupt mid-flow step answers); "Hi"/"Hello"/"Hey"/"Demo" now always restart the flow even mid-flow; scheduler added to `scripts/start.sh` so DB keep-alive ping runs on production too. |
