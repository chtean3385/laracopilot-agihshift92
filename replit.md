...
| 2026-06-04 | v1.0.36 | `pending` | Document upload: 1MB server-side limit (was 5MB) across DocumentController, BookingGuestController, CustomerController, GuestCheckinController; client-side Canvas compression on upload form — images auto-resized to max 1400px and compressed to JPEG 80% (55% fallback), shows before/after KB with savings %; PDFs validated size-only. |
