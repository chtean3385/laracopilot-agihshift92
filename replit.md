...
| 2026-06-04 | v1.0.34 | `pending` | Fix: Store all uploaded files as base64 in Neon DB (customer_documents.file_content, booking_guests.id_document_content, guest_checkin_requests.id_document_content, customers.id_document_content) — files now survive every deployment. Documents index shows "Re-upload" banner for old missing files instead of broken image. |
