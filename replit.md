...
- **Nginx location block order — Livewire MUST come first:** In `scripts/nginx.conf.template`, the `location ~ ^/livewire-[^/]+/` block must appear **before** the static-file regex `location ~* \.(css|js|...)$`. Nginx matches regex locations in order of appearance — first match wins. If the static-file rule comes first, `/livewire-*/livewire.js` matches `\.js$` and returns 404, silently breaking ALL Livewire components (search, pagination, `wire:click`).
- **Rooms: filter both `status != 'inactive'` AND `is_active = true`:** The `rooms` table tracks inactive state two ways — `status = 'inactive'` (set via the UI Status dropdown) and `is_active` boolean column (added 2026-06-08). All existing rows got `is_active = true` by default, so `is_active` alone won't exclude UI-deactivated rooms. Booking dropdowns must always filter both: `->where('status', '!=', 'inactive')->where('is_active', true)`.
- **Fetch calls without `Accept: application/json` get HTML errors:** Any `fetch()` call to a Laravel endpoint that omits the `Accept: application/json` header will receive an HTML error page (419 CSRF, 500, 404) instead of JSON when something goes wrong. Calling `.json()` on that HTML response throws a parse error and shows garbled output. Always add `'Accept': 'application/json'` to all fetch headers.
- **Production nginx/WAF blocks POST URLs containing "signature":** The live site's WAF returns 403 for POST requests whose URL contains the word `signature`. Use `/sign` instead — primary guest route is `POST /customers/{customerId}/sign` (`customers.signature`), booking guest route is `POST /bookings/{id}/guests/{guestId}/sign` (`booking.guests.signature`). Dev works fine (php artisan serve bypasses nginx). Always use Laravel's `route()` helper in Blade fetch calls to avoid hardcoding paths.
...
| 2026-06-04 | v1.0.33 | `3181f43` | Fix: PHP-FPM pool upload limits — upload_max_filesize 2M→20M, post_max_size 8M→25M, memory_limit 128M→256M (fixes "file failed to upload" on document upload). |
| 2026-06-04 | v1.0.34 | `a20705c` | Fix: Store all uploaded files as base64 in Neon DB (customer_documents.file_content, booking_guests.id_document_content, guest_checkin_requests.id_document_content, customers.id_document_content) — files now survive every deployment. Documents index shows "Re-upload" banner for old missing files instead of broken image. |
| 2026-06-04 | v1.0.35 | `778459f` | Fix: nginx SCRIPT_FILENAME uses $document_root instead of $realpath_root — on Replit Reserved VM the workspace path is a symlink so $realpath_root resolved to a path PHP-FPM rejected with 403. |
| 2026-06-04 | v1.0.36 | `9c5dac8` | Document upload: 1MB server-side limit (was 5MB) across DocumentController, BookingGuestController, CustomerController, GuestCheckinController; client-side Canvas compression on upload form — images auto-resized to max 1400px and compressed to JPEG 80% (55% fallback), shows before/after KB with savings %; PDFs validated size-only. |
| 2026-06-04 | v1.0.37 | `pending` | Fix: deployment crash loop — nginx was launched via `nix-shell -p nginx` which doesn't exist in production VM containers; added nginx as a Nix system dependency (replit.nix) so it's in PATH in both dev and prod, then changed start.sh to call `nginx` directly. Deployment target changed from autoscale → reserved_vm. |
| 2026-06-08 | v1.0.38 | `23c73dd` | Bug fixes: inactive rooms no longer appear in booking dropdowns (`status != maintenance/inactive`); blank Total Amount on bookings/create fixed (calculateTotal called after room dropdown rebuild); signature save returning garbled HTML fixed (`Accept: application/json` added to fetch calls); build.sh order fixed (safe-migrate before optimize so view cache survives). |
| 2026-06-08 | v1.0.39 | `1c11075` | Fix 500 error on bookings/create: added missing `is_active` boolean column to `rooms` table (migration); updated Room model fillable + casts; booking dropdowns now filter `status != inactive` AND `is_active = true`; nginx Livewire location block moved before static-file regex rule — fixes Mark Available and pagination on production (Livewire JS was returning 404). |
| 2026-06-08 | v1.0.40 | `614ed87` | Docs: added 4 new Gotchas (nginx location block order, rooms dual-inactive filtering, fetch Accept header requirement); release history updated. |
