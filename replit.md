...
- **Production nginx/WAF blocks POST to `/guests/*` paths:** The live site's nginx or upstream WAF returns 403 for POST requests whose URL starts with `/guests/`. Use `/customers/` prefix instead — the primary guest signature route is `POST /customers/{customerId}/signature` (`customers.signature`). Dev works fine (php artisan serve bypasses nginx). Always use Laravel's `route()` helper in Blade fetch calls to avoid hardcoding paths.
