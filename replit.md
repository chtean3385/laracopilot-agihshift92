...
- **Production nginx/WAF blocks POST to `/guests/*` and `/customers/*` paths:** The live site's WAF returns 403 for POST requests to those prefixes. Route guest-related POSTs through `/bookings/` instead — primary guest sign is `POST /bookings/{bookingId}/primary-sign` (`customers.signature`), booking guest sign is `POST /bookings/{id}/guests/{guestId}/sign` (`booking.guests.signature`). Dev works fine (php artisan serve bypasses nginx). Always use Laravel's `route()` helper in Blade fetch calls to avoid hardcoding paths.
